...
V1LayerParameter_LayerType UpgradeV0LayerType(const string& type) {
  ...
  } else if (type == "accuracy_distance") {
    return V1LayerParameter_LayerType_ACCURACYDISTANCE;
  } else if (type == "euclidean_weight_loss") {
    return V1LayerParameter_LayerType_EUCLIDEANWEIGHT_LOSS;
  } else if (type == "bnll") {
...
bool UpgradeV1LayerParameter(const V1LayerParameter& v1_layer_param,
  ...
  if (v1_layer_param.has_accuracy_distance_param()) {
    layer_param->mutable_accuracy_distance_param()->CopyFrom(
        v1_layer_param.accuracy_distance_param());
  }
  if (v1_layer_param.has_euclidean_weight_loss_param()) {
    layer_param->mutable_euclidean_weight_loss_param()->CopyFrom(
        v1_layer_param.euclidean_weight_loss_param());
  }
...
const char* UpgradeV1LayerType(const V1LayerParameter_LayerType type) {
  ...
  case V1LayerParameter_LayerType_ACCURACY:
    return "Accuracy";
  case V1LayerParameter_LayerType_ACCURACYDISTANCE:
    return "AccuracyDistance";
  case V1LayerParameter_LayerType_EUCLIDEANWEIGHT_LOSS:
    return "EuclideanWeightLoss";
